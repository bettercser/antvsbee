...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # BEGIN Problem 3 and 4
         # REPLACE THIS LINE
        cur_lower_bound = self.lower_bound
        cur_upper_bound = self.upper_bound
        nearest_bee_place = self.place
        while not nearest_bee_place.is_hive:
            cur_upper_bound -= 1
            cur_lower_bound -= 1
            if cur_upper_bound < -1:
                 
                break
            if cur_lower_bound > -1:
                nearest_bee_place = nearest_bee_place.entrance
                continue

            if nearest_bee_place.bees:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        explore_damage = amount
        cur_pos_bees = self.place.bees[:]
        if self.health - amount <= 0:
            explore_damage += self.damage
        # END Problem 5
        super().reduce_health(amount)
        for target in cur_pos_bees:
            if target is not None:
                target.reduce_health(explore_damage)
# BEGIN Problem 6
# The WallAnt class
# END Problem 6
class WallAnt(Ant):
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, target):
        """Eat a bee which at the current place"""
        if target is not None:
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        queen_cur_pos = self.place.exit
        while queen_cur_pos is not None:
            
            if queen_cur_pos.ant is not None:
                queen_cur_pos.ant.double()
                if queen_cur_pos.ant.is_container:
                    if queen_cur_pos.ant.ant_contained is not None:
                        queen_cur_pos.ant.ant_contained.double()
            queen_cur_pos = queen_cur_pos.exit
        super().action(gamestate)
        # END Problem 12
...
class LaserAnt(ThrowerAnt):
        ...
        while not cur_place.is_hive:
            if cur_place.ant:
                if cur_place.ant.is_container:
                    all_insect[cur_place.ant] = distance
                    if not cur_place.ant.ant_contained is self and cur_place.ant.ant_contained:
                        all_insect[cur_place.ant.ant_contained] = distance
                else:
                    if not cur_place.ant is self:
                        all_insect[cur_place.ant] = distance
            if cur_place.bees:
                for target in cur_place.bees:
                    if target:
                        all_insect[target] = distance
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
            ...
            self.damage = self.damage - distance * 0.25 - self.insects_shot * 0.0625
            if self.damage <= 0:
                return 0
            else:
                return self.damage
